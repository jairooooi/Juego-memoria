...
int main(){
    ...
    //Interfaz
    Interfaz interfaz;
    interfaz.LeerPuntajesDesdeArchivo();
    while(window.isOpen()){
        Event event;
        ...
        // Guardar puntajes al finalizar un juego
        if (interfaz.ConsultarDescubiertas() == 12 || interfaz.ConsultarTiempo() < 0) {
            // Puedes agregar esta lógica después de que el juego termine
            Puntaje nuevoPuntaje("NombreJugador", interfaz.ConsultarPuntaje());
            interfaz.AgregarPuntaje(nuevoPuntaje);
            interfaz.GuardarPuntajesEnArchivo();
            // También puedes reiniciar el juego o realizar otras acciones según sea necesario
        }
